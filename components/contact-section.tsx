...
export function ContactSection() {
  ...
  return (
    <section className="px-4 sm:px-6 md:px-12 lg:px-20 py-16 md:py-32 relative" data-inspectable>
      {/* Large typography */}
      ...
      <div className="max-w-xl mb-8">
        <p className="text-base md:text-lg lg:text-xl text-muted-foreground leading-relaxed mb-6 md:mb-8">
          Currently open for new opportunities. Whether you have a question or just want to say hi, I'll try my best to
          get back to you!
        </p>
        ...
        <div className="flex flex-col sm:flex-row flex-wrap gap-3">
          <MagneticButton as="a" href={SOCIAL_LINKS.email} strength={0.2}>
            <div className="inline-flex items-center justify-center gap-2 md:gap-3 bg-foreground text-background px-4 md:px-6 py-3 md:py-4 rounded-full font-medium text-xs md:text-sm tracking-wide hover:scale-105 transition-transform duration-300 w-full sm:w-auto">
              ...
              <span className="truncate">{SITE_CONFIG.email.toUpperCase()}</span>
            </div>
          </MagneticButton>
          <div className="flex gap-3">
            <MagneticButton as="a" href={SOCIAL_LINKS.linkedin} external strength={0.4}>
      ...
      <div className="mt-12 md:mt-0 md:absolute md:right-6 lg:right-12 xl:right-20 md:bottom-20 text-left md:text-right">
        <div className="text-[10px] md:text-xs text-muted-foreground tracking-widest mb-1">LOCATION</div>
        <div className="font-semibold text-sm md:text-base mb-4">{SITE_CONFIG.location}</div>
        <div className="flex items-center gap-2 md:justify-end">
          <div className="w-px h-6 md:h-8 bg-foreground" />
          <div>
            <div className="text-[10px] md:text-xs text-muted-foreground tracking-widest mb-1">LOCAL TIME</div>
            <div className="font-semibold font-mono tabular-nums text-sm md:text-base">{localTime}</div>
          </div>
        </div>
      </div>
